...
<title>Alterar lista de favoritos - BusEye</title>
...
<body>
  ...
  <main class="py-3">
  <div class="container">
  	<h5 class="ps-3 mb-2">Alterando lista de favoritos ID ${listaFavoritos.idLista}</h5>

	<c:if test="${repetido == true}">
	  <div class="d-flex justify-content-center mb-2">
		<p class="fs-5 bg-danger p-2 m-0 rounded branco">Lista de favoritos já existente</p>
	  </div>
	</c:if>

    <form class="p-3 rounded" action="/Sprint4-POO/alterarListaFavoritos" method="post">
      <div id="form" class="mb-5">
          <div class="mb-3">
          	<input type="hidden" name="id" value="${listaFavoritos.idLista}">
            ...
            <input id="idLinha" class="int form-control" type="number" name="idLinha" value="${listaFavoritos.idLinha}" min="1" required>
          </div>
          ...
          <div>
			...
			<c:choose>
			  ...
			  <c:when test="${naoEncontradoU == true}">
              	<div class="fw-bold" style="color: red">Usuário não encontrado</div>
			  </c:when>
			</c:choose>

            <input id="idUsuario" class="int form-control" type="number" name="idUsuario" value="${listaFavoritos.idUsuario}" min="1" required>
          </div>
      </div>
      ...
      <div class="d-flex flex-wrap justify-content-between text-center botoes">
        ...
        <div>
          ...
          <a href="/Sprint4-POO/listaListaFavoritos" class="btn btn-primary">Voltar</a>
        </div>
      </div>
    </form>
  </div>
