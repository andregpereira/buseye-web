...
<title>Listas de favoritos - BusEye</title>
...
<body>
  ...
  <main class="px-4 py-3">
	<h5 class="ps-3 mb-2">Listas de favoritos</h5>

	<c:if test="${alterado == true}">
	  <div class="d-flex justify-content-center mb-2">
	  	<p class="fs-5 bg-success p-2 m-0 rounded branco">Lista de favoritos ID ${listaFav.idLista} alterada</p>
	  </div>
	</c:if>

	<c:if test="${removido == true}">
	  <div class="d-flex justify-content-center mb-2">
	  	<p class="fs-5 bg-danger p-2 m-0 rounded branco">Lista de favoritos ID ${listaFav.idLista} removida</p>
	  </div>
	</c:if>
	...
	<c:choose>
	<c:when test="${not empty listaFavoritos}">
	<table class="table table-light table-striped table-hover table-bordered border-dark mb-2">
	  ...
	  <tbody>
	  <c:forEach items="${listaFavoritos}" var="favoritos">
		<tr>
		  <td>${favoritos.idLista}</td>
		  <td>${favoritos.idLinha}</td>
		  <td>${favoritos.idUsuario}</td>
		  <td>
			<div class="d-flex justify-content-around">
			  <a href="/Sprint4-POO/formAlterarListaFavoritos?id=${favoritos.idLista}" title="Editar">
				<span class="fas fa-edit acoes"></span>
			  </a>
			  <a href="/Sprint4-POO/removerListaFavoritos?id=${favoritos.idLista}"  title="Remover">
	...
	<c:otherwise>
	  <p class="fs-2 mb-3 text-center branco">Não há registros de listas de favoritos</p>
	</c:otherwise>
	</c:choose>
  ...
  <div class="d-flex justify-content-end mt-3">
	<a href="/Sprint4-POO/formListaFavoritos" class="btn btn-primary">Voltar</a>
  </div>
